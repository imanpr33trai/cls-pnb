...
case preg_match('/^\/admin/', $request_uri):
    ...
    exit;
       ...
       case preg_match('/^\/article\/([a-zA-Z0-9-_]+)\/?$/', $request_uri, $matches):
        ...
        $_GET['slug'] = $matches[1];        include __DIR__ . '/app/pages/single-article.php';
        break;

       case preg_match('/^\/category\/([a-zA-Z0-9-_]+)\/?$/', $request_uri, $matches):


        $_GET['slug'] = $matches[1];        include __DIR__ . '/app/pages/single-category.php';
        break;

       case preg_match('/^\/ads\/([a-zA-Z0-9-_]+)\/?$/', $request_uri, $matches):


        $_GET['slug'] = $matches[1];        include __DIR__ . '/app/pages/single-ad.php';
        break;
