...
$stmt = $conn->prepare("SELECT * FROM ads WHERE subcategory_id = ? AND LOWER(status) = 'live' ORDER BY id DESC");
...
if ($result->num_rows > 0) {
    ...
    while ($post = $result->fetch_assoc()) {
        ...
        $post_image = !empty($post['image']) ? $base_url . 'assets/uploads/' . $post['image'] : $base_url . 'assets/images/default-ad.png';
        $post_url = $base_url . $post['id']; // You can make this URL pretty later too!
        ?>
        <div class="col-12 col-md-6 col-lg-4 mb-4">
            <div class="card h-100">
                <a href="<?= $post_url ?>">
                    <img src="<?= htmlspecialchars($post_image) ?>" class="card-img-top" alt="<?= htmlspecialchars($post['title']) ?>">
                </a>
                <div class="card-body">
                    <h5 class="card-title"><a href="<?= $post_url ?>" class="text-decoration-none text-dark"><?= htmlspecialchars($post['title']) ?></a></h5>
                    <p class="card-text text-muted"><?= htmlspecialchars(substr($post['description'], 0, 80)) ?>...</p>
                    <p class="card-text fw-bold">Price: $<?= htmlspecialchars($post['price']) ?></p>
                </div>
            </div>
        </div>
        <?php
